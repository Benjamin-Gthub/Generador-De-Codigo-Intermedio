...
def GenerarCodigoIntermedio(Archivo):
    #Abrir el archivo:
    Archivo=open(Archivo,'r',encoding='utf-8')

    #extraer las lineas(se extrae cada linea con el simbolo \n )
    lectura=Archivo.readlines()
    
    #Se eliminan los simboos \n y se convierten en un arreglo
...
os.system('cls')#Limpiar la pantalla y mostrar
CodigoLine,A_Saltos,A_EsCeros=GenerarCodigoIntermedio("Archivo.txt")
#print(tabulate(CodigoLine,headers=["Dir","Operador","Var1","Var2","Resultado"],tablefmt='fancy_grid'))

#SE AGREGA LAS DIRECCIONES
TablaConDirecciones=AgregarDirecciones(CodigoLine,A_EsCeros,A_Saltos)

print(tabulate(TablaConDirecciones,headers=["Dir","Operador","Var1","Var2","Resultado"],tablefmt='fancy_grid'))
...
#Ejemplo:
#C_I_Secuencial("a =      3  * 5+ 5")

##*cadena=str(input("Digite la expresion: "))
##*print("La expresion en arreglo, es:",GenerarArreglo(cadena))
##*print("La notacion postfija, es:",infixToPostfix(GenerarArreglo(cadena)))
#print("La notacion postfija en forma de arreglo, es:",infixToPostfix(GenerarArreglo(cadena)).split())
